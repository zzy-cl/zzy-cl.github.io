---
title: 日常动态
date: 2023-03-17 01:04:15
aside: false
---
<div id="talk">
    <div class='loading'><img src="https://imagecloud.zhaozeyu.top/2023/03/17/6413572403637.gif" alt="加载中..."></div>
</div>
...
<script>
    ...
    // 页面说说
    function pageTalk() {
        fetch('https://memos.zhaozeyu.top/api/memo?creatorId=1&tag=动态&limit=30').then(res => res.json()).then(data => { // 注意修改域名和用户id
            let items = [],
                html = '',
            ...
            items.forEach(item => {
                html += `<div class="talk_item"><div class="talk_meta"><img class="no-lightbox no-lazy avatar" src="https://imagecloud.zhaozeyu.top/2023/03/10/640afab14b960.jpg" alt="avatar"><div class="info"><span class="talk_nick">是zzy呀${icon}</span><span class="talk_date">${item.date}</span></div></div><div class="talk_content">${item.content}</div><div class="talk_bottom"><div><span class="talk_tag"># ${item.tag}</span></div></div></div>` // 注意修改头像链接和名称
            })
    ...
    function Format(item) {
        ...
        if (imgls) {
            ...
            imgls.map(item => { return item.replace(/!\[.*\]\((.*?)\)/, '$1') }).forEach(e => content += `<a href="${e}" data-fancybox="gallery" class="fancybox" data-thumb="${e}"><img class="no-lazy" src="${e}" alt=""></a>` // 2023-02-06更新
            )
            content += '</div>'
        }
        return {
